...
$(function() {
  ...
  $("button#updateAddFromServer").button({
    icons: { primary: "ui-icon-eject" } // Icône de disquette sur le bouton AddTag
  });
});
...
function errorNotif(titre,message){
  $("#notification").notify("create", "error", {
    title: titre,
    text: message
  },{expires: false}); // Les erreurs sont sticky
}
...
$(function() {
  ...
  $( "#accordion" ).accordion({active: -1 }); // Pass -1 to close all (only possible with collapsible:true )
});
...
function getStatus()
{
  ...
  $.ajax({
    ...
    datatype: 'json',
    data: { timestamp: timestamp, line: line },
